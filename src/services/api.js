...
export default http;
// ─────────────────────────────────────────────────────────────────────────────
// RECONCILIATION API  (endpoints planned — errors surface to the UI, no mocks)
// ─────────────────────────────────────────────────────────────────────────────
export const reconAPI = {
  listRuns:      (params) => http.get('/recon/runs/', { params }),
  getDelta:      (id)     => http.get(`/recon/runs/${id}/delta/`),
  getMismatches: (params) => http.get('/recon/mismatches/', { params }),
  raiseDispute:  (id)     => http.post(`/recon/mismatches/${id}/dispute/`),
  triggerRun:    ()       => http.post('/recon/trigger/'),
};

// ─────────────────────────────────────────────────────────────────────────────
// WEBHOOK API  (endpoints planned — errors surface to the UI, no mocks)
// ─────────────────────────────────────────────────────────────────────────────
export const webhookAPI = {
  listEvents:  (params) => http.get('/webhooks/events/', { params }),
  getDLQ:      (params) => http.get('/webhooks/dlq/', { params }),
  replayEvent: (id)     => http.post(`/webhooks/dlq/${id}/replay/`),
  getStats:    ()       => http.get('/webhooks/stats/'),
};

// ─────────────────────────────────────────────────────────────────────────────
// APPROVALS API  (endpoints planned — errors surface to the UI, no mocks)
// ─────────────────────────────────────────────────────────────────────────────
export const approvalsAPI = {
  getQueue: (params) => http.get('/approvals/queue/', { params }),
  approve:  (id, d)  => http.post(`/approvals/${id}/approve/`, d),
  reject:   (id, d)  => http.post(`/approvals/${id}/reject/`, d),
  getRules: ()       => http.get('/approvals/rules/'),
};
