...
const NAV = [
  ...
  {
    id: 'redis', label: 'Redis Inspector', to: '/dev/redis', badge: 'SOON',
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
  ...
  {
    id: 'config', label: 'Env Config', to: '/dev/config', badge: 'SOON',
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
        <circle cx="12" cy="12" r="3"/>
  ...
  {
    id: 'indexes', label: 'Index Health', to: '/dev/indexes', badge: 'SOON',
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
...
const S = {
  ...
  sidebar: {
    ...
    background: 'rgba(8,12,20,0.97)', borderRight: `1px solid ${C.border}`,
  ...
  logoMark: {
    ...
    fontSize: 16, color: '#fff', boxShadow: '0 0 16px rgba(59,97,245,0.4)',
  },
  ...
  logoBadge: { fontSize: 9, fontWeight: 700, color: C.brand, letterSpacing: '0.1em', background: C.brandGlow, padding: '1px 5px', borderRadius: 3, width: 'fit-content' },
  collapseBtn: {
  ...
  statusDot:  { width: 6, height: 6, borderRadius: '50%', background: C.success, boxShadow: `0 0 6px ${C.success}`, animation: 'pulseGlow 2s ease-in-out infinite' },
  ...
  navLink: {
    ...
    borderRadius: 8, color: C.textSec, textDecoration: 'none',
    fontSize: 13.5, fontWeight: 500, transition: 'background 150ms, color 150ms',
    position: 'relative', animation: 'slideIn 300ms both',
    whiteSpace: 'nowrap', overflow: 'hidden', border: '1px solid transparent',
  },
  navLinkActive: { background: C.brandGlow, color: '#8ba7ff', border: `1px solid rgba(59,97,245,0.2)` },
  ...
  backBtn: {
    ...
    fontSize: 12, fontFamily: "'DM Sans',sans-serif", transition: 'background 150ms, color 150ms',
  },
  sidebarFooter: { display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderTop: `1px solid ${C.border}` },
  avatar: {
  ...
  logoutBtn: { background: 'transparent', border: 'none', cursor: 'pointer', color: C.textMuted, padding: 4, borderRadius: 4, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'color 150ms' },
  main:      { flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative', zIndex: 5 },
  topBar: {
  ...
  envChip: {
    display: 'flex', alignItems: 'center', gap: 6, background: C.bgCard,
    border: `1px solid ${C.border}`, borderRadius: 6, padding: '4px 10px',
    fontSize: 11, color: C.textSec, fontFamily: "'JetBrains Mono',monospace",
  },
...
export default function DevLayout() {
  ...
  // Dev panel is ALWAYS dark — force it, restore previous theme on unmount
  useEffect(() => {
          ...
          {NAV.map((item, i) => {
            if (item._divider) {
              ...
              return (
                <div key={item.label} style={{ padding: '10px 10px 4px', marginTop: 4 }}>
                  ...
                  <span style={{ fontSize: 9, fontWeight: 700, color: C.textMuted, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                    {item.label}
                  </span>
                </div>
              );
            ...
            return (
              <NavLink
                ...
                style={({ isActive }) => ({
                  ...S.navLink,
                  ...(isActive && !isSoon ? S.navLinkActive : {}),
              ...
              >
                {({ isActive }) => (
                  <>
                    <span style={{ ...S.navIcon, ...(isActive && !isSoon ? S.navIconActive : {}) }}>
                      {item.icon}
                    </span>
                    ...
                    {!collapsed && item.badge && (
                      <span style={{
                        ...
                        background: item.badge === 'NEW' ? 'rgba(59,97,245,0.25)' : 'rgba(255,255,255,0.06)',
                        color: item.badge === 'NEW' ? '#8ba7ff' : C.textMuted,
                      }}>
                        {item.badge}
                      </span>
                    )}
                    {isActive && !collapsed && !isSoon && <span style={S.navPip} />}
                  </>
                )}
              </NavLink>
            );
