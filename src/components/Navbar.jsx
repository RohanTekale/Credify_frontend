// src/components/Navbar.jsx — UPDATED: user avatar dropdown + dashboard link
import React, { useState, useEffect, useRef } from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import { CreditCard, Menu, X, LayoutDashboard, LogOut, User, ChevronDown, Bell } from 'lucide-react';
import useAuthStore from '../store/authStore';

const avatarPalette = ['#3b61f5', '#8b5cf6', '#10b981', '#f59e0b', '#ec4899', '#06b6d4'];

const DropdownItem = ({ icon: Icon, label, onClick, color = '#aaa', danger }) => (
  <button
    onClick={onClick}
    style={{
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              <button key={link.path} onClick={() => navigate(link.path)} className="btn-ghost"
                style={{ color: isActive(link.path) ? '#fff' : '#aaa' }}>
                {link.label}
              </button>
            ))}
            {user && (
          ...
          <div style={{ padding:16, background:'rgba(8,12,20,0.98)', borderTop:'1px solid rgba(255,255,255,0.07)' }}>
            {navLinks.map((link) => (
              <button key={link.path} onClick={() => navigate(link.path)} style={{
                ...
                background: isActive(link.path) ? 'rgba(59,97,245,0.12)' : 'transparent',
